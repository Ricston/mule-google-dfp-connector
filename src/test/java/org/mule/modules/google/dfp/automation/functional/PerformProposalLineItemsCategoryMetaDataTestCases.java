...
public class PerformProposalLineItemsCategoryMetaDataTestCases extends AbstractTestCase<GoogleDfpConnector> {

    public PerformProposalLineItemsCategoryMetaDataTestCases() {
    ...
    public void testMetaDataKeys() throws Exception {
        ...
        for (MetaDataKey key : metaDataKeys) {
            if (key.getId()
                    .equals("Archive Proposal Line Items"))
                archive = key;
            if (key.getId()
                    .equals("Unarchive Proposal Line Items"))
                unarchive = key;
        }
