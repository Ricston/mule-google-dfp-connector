...
public class PerformOrdersCategoryMetaDataTestCases extends AbstractTestCase<GoogleDfpConnector> {

    public PerformOrdersCategoryMetaDataTestCases() {
    ...
    public void testMetaDataKeys() throws Exception {
        ...
        for (MetaDataKey key : metaDataKeys) {
            if (key.getId()
                    .equals("Archive Orders"))
                archive = key;
            if (key.getId()
                    .equals("Unarchive Orders"))
                unarchive = key;
        }
