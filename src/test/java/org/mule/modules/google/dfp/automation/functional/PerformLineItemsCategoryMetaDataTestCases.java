...
public class PerformLineItemsCategoryMetaDataTestCases extends AbstractTestCase<GoogleDfpConnector> {

    public PerformLineItemsCategoryMetaDataTestCases() {
    ...
    public void testMetaDataKeys() throws Exception {
        ...
        for (MetaDataKey key : metaDataKeys) {
            if (key.getId()
                    .equals("Archive Line Items"))
                archive = key;
            if (key.getId()
                    .equals("Unarchive Line Items"))
                unarchive = key;
        }
